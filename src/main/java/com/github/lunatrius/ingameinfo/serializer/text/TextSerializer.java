...
public class TextSerializer implements ISerializer {
	...
	public boolean save(File file, Map<Alignment, List<List<Value>>> format) {
		try {
			...
			Reference.logger.log(Level.SEVERE, "Could not save text configuration file!", e);
		}
	...
	private void writeValue(BufferedWriter writer, Value value) throws IOException {
		...
		if (size == 0) {
			...
			default:
				Reference.logger.log(Level.SEVERE, String.format("Unknown value type %s! This is a bug!", type));
				break;
			}
		} else {
			Reference.logger.log(Level.SEVERE, String.format("Invalid size %d for value type %s!", size, type));
		}
	}
}
